...
// Exits the program with internal compiler error if `index` points to memory
// outside of the vector's inner buffer, or vec is NULL
//
// XXX: On invalid accesses, we choose to exit the program instead of
...
//      So if this function ever exits, that's because of _an actual bug_.
void vec_validate_idx(vector* vec, size_t index) {
    if (vec == NULL) {
        fprintf(stderr, "trying to index NULL vec");
        _Exit(INTERNAL_ERR);
    }
    ...
    fprintf(stderr, "index out of bounds: %lu.", index);
    _Exit(INTERNAL_ERR);
}
...
// Function for convenient pushing of 'char'
char* vec_pushchar(vector* vec, const char c) {
    return (char*)vec_push(vec, (void*)&c);
}
...
//
// Will exit with internal error if the index is out of bounds.
...
// The pointer to the vector will be set to `NULL`.
...
// TRUNCATING IT TO ITS CAPACITY BEFOREHAND.
//
...
char* vec_get_str(vector* vec) {
    ...
    char* str = (char*)calloc(vec->len + 1, sizeof(char));
